...
void soft_dac_initialize( void ) {
  ...
  /* Initialize OC0A and OC0B pins (PB0 and PB1) */
  DDRB |= (1 << PIN1) | (1 << PIN0);     // PB0 and PB1 as output
  PORTB &= ~((1 << PIN1) | (1 << PIN0)); // PB0 and PB1 at low state
  
  /* Initialize Timer 0 (for fast PWM) */
  // OC0A connected to PB0 w/ "Clear on compare match, set at bottom"
  ...
  OCR0A = 0; // Set defaut output channel A value to 0
  OCR0B = 0; // Set defaut output channel B value to 0
